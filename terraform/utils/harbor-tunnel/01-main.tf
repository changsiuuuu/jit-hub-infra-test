...
# Harbor 전용 터널 + DNS(harbor.leechs.shop) + 온프레 상시 connector
#
# 모듈 경로 주의:
...
#    - dns_records = ["harbor"]  ->  harbor.leechs.shop CNAME 생성
#    - ingress_rule: harbor.leechs.shop  ->  cloudflared Pod 가 Harbor 사설IP로 전달
# -----------------------------------------------------------------------------
module "harbor_tunnel" {
  ...
  domain_name = var.domain_name # leechs.shop

  # harbor 서브도메인만 생성 (서비스용 @/argocd/grafana 와 무관)
  dns_records = [var.harbor_subdomain] # ["harbor"] -> harbor.leechs.shop

  ingress_rules = [
    {
      hostname = "${var.harbor_subdomain}.${var.domain_name}" # harbor.leechs.shop
